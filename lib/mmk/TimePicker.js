...
export class TimePicker {
  constructor(options = {}) {
    ...
    this.container = options.container ?? null; // Fixed container for API 3.0
  ...
  render() {
    ...
    // Create fixed container if not provided (for overlay behavior)
    if (!this.container) {
  ...
  renderTimeDisplay(centerX, y) {
    ...
    // Hour digit 1
    this.hourDigit1 = this.container.createWidget(hmUI.widget.TEXT, {
      x: startX,
      y: y,
      w: digitWidth,
      h: height,
    ...
    // Hour digit 2
    this.hourDigit2 = this.container.createWidget(hmUI.widget.TEXT, {
      x: startX + digitWidth,
      y: y,
      w: digitWidth,
      h: height,
    ...
    // Colon
    const colon = this.container.createWidget(hmUI.widget.TEXT, {
    ...
    // Minute digit 1
    this.minDigit1 = this.container.createWidget(hmUI.widget.TEXT, {
    ...
    // Minute digit 2
    this.minDigit2 = this.container.createWidget(hmUI.widget.TEXT, {
  ...
  createKeypadButton(x, y, size, text, callback, fontSize = 32) {
    const btn = this.container.createWidget(hmUI.widget.BUTTON, {
      x: x,
      y: y,
      w: size,
      h: size,
  ...
  renderOkButton(centerX, y) {
    const btnWidth = 80;
    const btnHeight = 36;
    const okBtn = this.container.createWidget(hmUI.widget.BUTTON, {
  ...
  confirm() {
    // Guard against multiple rapid-fire calls
    if (this.confirmed) {
      return;
    }
    this.confirmed = true;
  ...
  destroy() {
    ...
    // Destroy container if we created it
    if (this.ownContainer && this.container) {
